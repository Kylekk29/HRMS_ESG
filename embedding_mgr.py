...
class EmbeddingManager:
    ...
    def __init__(self):
        ...
        logger.info("Embedding Manager initialized (model loads on first use)")
        logger.info("=" * 60)
    ...
    @property
    def embeddings(self):
        """Lazy-load the embedding model — ~2-4s on first access."""
        if self._embeddings is None:
            if not self._load_started:
                self._load_started = True
                logger.info(f"Lazy-loading embedding model: {config.EMBEDDING_MODEL_NAME}")
                ...
                logger.info(f"Embedding model loaded in {self._load_time:.2f}s")
            else:
                if self._embeddings is None:
    ...
    def embed_cv_batch(
        ...
        """
        Embed multiple CV files using parallel workers.
        
        Each item: {"file_path": str, "file_name": str}
        
        When `jd` is provided, also runs similarity search and returns candidate
        text sections directly — avoids reloading FAISS indices from disk.
        ...
        Each result dict gains:
          - "file_name"
          - "file_id"
          - "candidate_section" (str) — only when `jd` is provided and no error
          - "faiss_db" (FAISS | None) — in-memory DB object when newly embedded;
            None when the index was reused from cache.
        """
        ...
        results: List[Dict] = [None] * len(cv_files_limited)

        max_workers = min(3, len(cv_files_limited))

        def process_one(cv: Dict, idx: int) -> Tuple[int, Dict]:
            ...
            logger.info(
                f"CV {idx+1}/{len(cv_files_limited)}: {cv['file_name']} "
                f"in {elapsed:.2f}s → {result.get('action', '?')}"
            )

            return (idx, result)

        if max_workers <= 1:
            for idx, cv in enumerate(cv_files_limited):
                _, result = process_one(cv, idx)
                results[idx] = result
        else:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_map = {
                    executor.submit(process_one, cv, idx): idx
                    for idx, cv in enumerate(cv_files_limited)
                }
    ...
    @timed
    def load_db(self, db_id: str, base_dir: str):
        """Load a FAISS index, checking in-memory cache first."""
        cached = self._faiss_cache.get(db_id)
        if cached is not None:
            logger.info(f"Using cached FAISS index: {db_id}")
            return cached

        db_path = os.path.join(base_dir, db_id)
        logger.info(f"Loading FAISS index from disk: {db_id}")
    ...
    @timed
    def load_employee_db(self, employee_id: str, doc_type: str = "profile"):
    ...
    def clear_faiss_cache(self) -> None:
        """Clear the in-memory FAISS cache to free memory."""
        count = len(self._faiss_cache)
        self._faiss_cache.clear()
        if count:
            logger.info(f"Cleared {count} FAISS index(es) from memory cache")
