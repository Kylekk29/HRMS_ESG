    ...
    def calculate_monthly_salary(self, employee_id: str, year: int, month: int) -> Dict:
        ...
        for req in lr_data.get("requests", []):
            if (
                ...
                e = req.get("end_date", "")
                if s and e and (s.startswith(period_prefix) or e.startswith(period_prefix)):
                    from hrms_manager import _daterange
                    days_in_range = _daterange(s, e)
                    deductible_in_month = sum(1 for d in days_in_range if d.startswith(period_prefix))
                    deductible_leave_days += deductible_in_month
